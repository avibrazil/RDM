...
int cmdline_main(int argc, const char *argv[]) {
  ...
  NSAutoreleasePool *pool = [NSAutoreleasePool new];
  {
    ...
    if (listDisplays) {
      for (int i = 0; i < nDisplays; i++) {
        ...
        fprintf(stdout,
                "Display %d: {resolution=%dx%d, scale = %.1f, freq = %d, "
                "bits/pixel = %d}\n",
                i, mode.derived.width, mode.derived.height,
                mode.derived.density, mode.derived.freq, mBitres);
      }
    ...
    if (listModes) {
      ...
      for (int i = 0; i < nModes; i++) {
        ...
        fprintf(stdout,
                "mode: {resolution=%dx%d, scale = %.1f, freq = %d, bits/pixel "
                "= %d}\n",
                mode.derived.width, mode.derived.height, mode.derived.density,
                mode.derived.freq, mBitres);
      }
  ...
  for (int i = 0; i < CFArrayGetCount(modes); i++) {
      CGDisplayModeRef mode = (CGDisplayModeRef)CFArrayGetValueAtIndex(modes,
  i); CFDictionaryRef infoDict = CGDisplayModeGetDictionary(mode);

      CFNumberRef resolution = (CFNumberRef) CFDictionaryGetValue(infoDict,
  CFSTR("kCGDisplayResolution")); CFNumberRef bits = (CFNumberRef)
  CFDictionaryGetValue(infoDict, CFSTR("BitsPerPixel")); float modeScale = 1.0f;
      int modeBitres;
      if(resolution)
                      CFNumberGetValue(resolution, kCFNumberFloatType,
  &modeScale); CFNumberGetValue(bits, kCFNumberIntType, &modeBitres);
  ...
  modeWidth, modeHeight, modeScale, modeBitres);
  }
  CFRelease(modes);
      return 0;
      */
...

    {
      ...
      for (int i = 0; i < nModes; i++) {
        ...
        iMode = i;
        break;
        // fprintf(stdout, "mode: {resolution=%dx%d, scale = %.1f, freq = %d,
        // bits/pixel = %d}\n", mode.derived.width, mode.derived.height,
        // mode.derived.density, mode.derived.freq, mode.derived.depth);
      }
